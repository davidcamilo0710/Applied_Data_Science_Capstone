...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        fig = px.pie(filtered_df[filtered_df['class'] == 1], values = 'class', names = 'Launch Site', title = 'Total Sucess Launches by Site')
        return fig
    else:
        data = filtered_df[filtered_df['Launch Site'] == entered_site]
        data = data.groupby(['class'])['Launch Site'].count().reset_index()
        fig = px.pie(data, values = 'Launch Site', names = 'class', title = 'Total Sucess Launches for site ' + entered_site)
        return fig
...
def get_scatter_chart(entered_site, entered_payload):
    ...
    if entered_site == 'ALL':
        ...
        data_scatter = data[data['Launch Site'] == entered_site]
    
    fig = px.scatter(data_scatter, x = 'Payload Mass (kg)', y = 'class', color = 'Booster Version Category')
